...
{   
  ...
  Serial.begin(9600);   // For Virtual Terminal display
  esp.begin(9600);      // For ESP8266

  // --- Display & Debug messages on Virtual Terminal ---
...
void loop()  
{
  int Sensor_Val = digitalRead(SENSOR_PIN);  // Get reading from microphone sensor
  ...
  if (Sensor_Val == HIGH) // Sound detected
  {
    ...
    Serial.println("Sound Detected! Sending data: 1");
  ...
  else // No sound
  {
    ...
    Serial.println("No sound detected. Sending data: 0");
